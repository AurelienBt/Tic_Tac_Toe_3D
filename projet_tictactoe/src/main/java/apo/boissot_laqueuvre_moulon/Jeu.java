...
public class Jeu {
     ...
     * @param protagonistes Types de joueurs : 0 : joueur VS joueur, 1 : joueur VS
     *                      IA, 2 : IA VS IA
     * @param quiCommence   Indique quel joueur commence à jouer : 0 : aléatoire, 1
     *                      : joueur 1, 2 : joueur 2
     ...
     * les classes nécessaires : Grille (2D ou 3D) et les joueurs, et en remplissant
     * la grille
     * avec les coups dejà réalisés
     * 
     * @param scanner    Scanner utilisé pour demander les inputs de l'utilisateur
     * @param sauvegarde Tableau de String contenant la sauvegarde précédemment
     *                   chargée
     */
    public void partie(Scanner scanner, ArrayList<String> sauvegarde) {
        ...
        if (erreurCoup) {
            ...
            if (coup[0].compareTo("X") == 0)
                jouerPartie(false, scanner);
            else
                jouerPartie(true, scanner);
        }
    }

    /***
     * Gère le lancement de la partie en instanciant les classes nécessaires :
     * Grille (2D ou 3D) et les joueurs
     * 
     * @param scanner Scanner utilisé pour demander les inputs de l'utilisateur
     */
     ...
     * gère l'enchainement des tours des joueurs et la vérification de la victoire
     * de l'un des joueurs
     * 
     * @param tourJ1  Booléen indiquant si c'est au tour du joueur 1 de placer un
     *                pion ou non
     * @param scanner Scanner utilisé pour demander les inputs de l'utilisateur
     */
    ...
    /***
     * Gère le déroulement du tour d'un joueur en faisant appel à une fonction
     * auxiliaire prenant en paramètre le prochain coup
     * 
     * @param tourJ1  Booléen indiquant si c'est au tour du joueur 1 de placer un
     *                pion ou non
     * @param scanner Scanner utilisé pour demander les inputs de l'utilisateur
    ...
    /***
     * Gère le déroulement du tour d'un joueur avec le choix de son coup et s'il
     * souhaite sauvegarder sa partie ou non
     * 
     * @param tourJ1       Booléen indiquant si c'est au tour du joueur 1 de placer
     *                     un pion ou non
     * @param scanner      Scanner utilisé pour demander les inputs de l'utilisateur
     * @param prochainCoup String du prochain coup joué par le joueur (si non nul,
     *                     ne demande pas d'input à l'utilisateur)
     * @return String contenant le coup joué par le joueur
     */
    private String jouerTour(boolean tourJ1, Scanner scanner, String prochainCoup) {
        ...
        if (tourJ1) {
            if (IAj1) {
                ...
                if (prochainCoup.compareTo("") != 0)
                    return jouerTour(tourJ1, scanner, prochainCoup);
            }
        ...
        // Tour du joueur 2
            ...
            if (IAj2) {
                ...
                do {
                    ...
                    if (prochainCoup.compareTo("") == 0)
                        input = this.j2.choisirCoup(text, scanner);
                    else
                        input = prochainCoup;
                    if (input.compareTo("save") == 0) {
                        sauvegarderPartie();
    ...
    /***
     * Sauvegarde la partie (les paramètres de la partie ainsi que les coups joués)
     * dans un fichier texte save.txt
     */
